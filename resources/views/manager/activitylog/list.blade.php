    ...
    <table class="table table-hover mb-0">
        ...
        @if(count($all))
            @foreach ($all as $key => $item)
                @php $module = '\App\\' . $item['model']; @endphp
                <tr>
                    <td>#{{ $item['id'] }}</td>
                    ...
                    <td>{{ $module::$modulename['fa'] }} ({{ $item['model'] }})</td>
                    <td><a href="{{ url('/'. App\Http\Controllers\HomeController::fetch_manager_pre_url() .'/' . $module::$modulename['en'] . '/' . $item['subject_id'] . '/edit') }}">#{{ $item['subject_id'] }}</a></td>
